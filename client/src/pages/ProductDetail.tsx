...
import { PRODUCTS, PRODUCTS_MAP } from '@/lib/productsData';

export default function ProductDetail() {
  const [, setLocation] = useLocation();
  const [match, params] = useRoute('/product/:id');
  const productId = params?.id || '1';
  const product = PRODUCTS_MAP[productId] || PRODUCTS_MAP['1'];
  ...
  const relatedProducts = PRODUCTS.filter((p) => p.id !== productId && p.category === product.category).slice(0, 3).length > 0
    ? PRODUCTS.filter((p) => p.id !== productId && p.category === product.category).slice(0, 3)
